...
export default function Navbar() {
  ...
  return (
    ...
    <div className={`sm:grid grid-cols-5 min-h-screen overflow-hidden font-verla ${theme}`}>
      {/* Mobile View */}
      <div className="sm:hidden relative flex w-screen h-12 shadow-md items-center md:px-10">
        ...
        {isToggled && (
          <div className={`overflow-hidden w-screen h-[100vh] top-12 absolute flex flex-col justify-between items-center`}>
            <ul className={`flex flex-col w-[90%] justify-center items-around gap-10 whitespace-nowrap shadow-sm mt-12 p-12 ${themeMode === 'dark' ? 'bg-secondary/5 transition-colors' : ''}`}>
              {/* Theme Toggle Button */}
              <button
                className="shadow-md rounded-full flex items-center justify-center h-full w-12 p-0 mx-2 transition-all outline-none hover:transition-all hover:bg-[#63a38a] hover:border-[#63a38a] hover:text-white"
      ...
      <div className="hidden col-span-1 shadow-2xl sm:flex flex-col items-center pt-5 bg-secondary/5">
        <div className="whitespace-nowrap h-12 w-24">
          {/* Theme Toggle Button */}
          <button
            className={`border border-primary shadow-md rounded-full flex items-center justify-center h-full w-full p-0 mx-2 transition-all outline-none hover:transition-all hover:bg-[#63a38a] ${theme === 'light' ? 'hover:border-[#63a38a]' : 'hover:border-[#63a38a] dark:border-[#63a38a]'} hover:text-white`}
            onClick={handleThemeChange}
          >
        ...
        <div className="flex flex-col justify-center w-full h-full px-3 my-10">
          <ul className="h-12 lg:h-16 w-full flex flex-col items-start justify-center gap-5">
            <NavbarItem navItems={navItems} themeMode={themeMode} />
            <ul className="flex gap-3 w-full justify-center mt-5">
              <li><Link to={'/'}><FacebookRoundedIcon /></Link></li>
